#!/usr/bin/env python3
"""
Comprehensive LLM-based adverse event extraction for ALL CMML papers
Runs on the complete dataset of 294 papers (169 Azacitidine + 125 Decitabine)
"""
...
class ComprehensiveLLMAdverseEventExtractor:
    def __init__(self, gemini_api_key: str):
        """Initialize the comprehensive LLM-based adverse event extractor"""
    ...
    def fetch_pmc_fulltext(self, pmcid: str) -> Optional[str]:
        ...
        try:
            ...
            # Don't print errors for every failed PMC fetch to reduce noise
            pass
...
FULL STUDY CONTENT:
{text_content[:80000]}  # Limit to avoid token limits
...
- For hematologic_ae, include: neutropenia, thrombocytopenia, anemia, leukopenia, pancytopenia, etc.
- For non_hematologic_ae, include: nausea, vomiting, fatigue, infection, fever, diarrhea, etc.
...
def main():
    """Main function to extract adverse events from ALL CMML studies using LLM"""
    ...
    try:
        extractor = ComprehensiveLLMAdverseEventExtractor(gemini_api_key)
        print("Successfully initialized comprehensive LLM adverse event extractor")
    except Exception as e:
        print(f"Failed to initialize extractor: {e}")
        return
    
    # Load study data
    input_file = 'pubmed_all_cmml_papers.json'
    if not os.path.exists(input_file):
        print(f"Input file {input_file} not found. Please run fetch_all_cmml_papers.py first.")
        return
    ...
    total_papers = sum(len(studies) for studies in study_data.values())
    processed_count = 0
    
    for drug_name, studies in study_data.items():
        print(f"\n{'='*60}")
        print(f"PROCESSING {drug_name.upper()} STUDIES")
        print(f"{'='*60}")
        print(f"Total papers: {len(studies)}")
        
        drug_adverse_events = []
        
        for i, study in enumerate(studies):
            processed_count += 1
            print(f"  Processing study {i+1}/{len(studies)} (Global: {processed_count}/{total_papers}): PMID {study.get('pmid', 'N/A')}")
            ...
            if ae_data.has_ae_data:
                ...
                if ae_data.ae_summary:
                    summary_preview = ae_data.ae_summary[:80] + "..." if len(ae_data.ae_summary) > 80 else ae_data.ae_summary
                    print(f"    Summary: {summary_preview}")
            else:
                print(f"    ○ No adverse event data found")
            
            # Add delay to avoid rate limiting - longer delay for comprehensive processing
            time.sleep(1.5)
            
            # Progress checkpoint every 50 papers
            if processed_count % 50 == 0:
                print(f"\n*** CHECKPOINT: {processed_count}/{total_papers} papers processed ({processed_count/total_papers*100:.1f}%) ***")
                
                # Save intermediate results
                intermediate_results = {k: v for k, v in all_adverse_events.items()}
                intermediate_results[drug_name] = drug_adverse_events
                
                with open('adverse_event_comprehensive_checkpoint.json', 'w') as f:
                    json.dump(intermediate_results, f, indent=2)
                print(f"*** Checkpoint saved to adverse_event_comprehensive_checkpoint.json ***\n")
        
        all_adverse_events[drug_name] = drug_adverse_events
        
        # Print drug summary
        studies_with_ae = sum(1 for study in drug_adverse_events if study.get('has_ae_data', False))
        print(f"\n{drug_name.upper()} SUMMARY:")
        print(f"Studies with AE data: {studies_with_ae}/{len(drug_adverse_events)} ({studies_with_ae/len(drug_adverse_events)*100:.1f}%)")
    
    # Save final results
    output_file = 'adverse_event_comprehensive.json'
    with open(output_file, 'w') as f:
        json.dump(all_adverse_events, f, indent=2)
    
    print(f"\n{'='*60}")
    print(f"COMPREHENSIVE ADVERSE EVENT EXTRACTION COMPLETED")
    print(f"{'='*60}")
    ...
    for drug_name, studies in all_adverse_events.items():
        ...
        print(f"\n{drug_name.capitalize()}: {studies_with_ae}/{len(studies)} studies with AE data ({studies_with_ae/len(studies)*100:.1f}%)")
        
        # Count specific AE types found
        specific_ae_count = sum(1 for study in studies if study.get('hematologic_ae') or study.get('non_hematologic_ae'))
        rates_found = sum(1 for study in studies if any([
            study.get('any_grade_ae_rate'),
            study.get('grade_3_4_ae_rate'),
            study.get('serious_ae_rate')
        ]))
        
        print(f"  - Studies with specific AE rates: {rates_found}")
        print(f"  - Studies with detailed AE breakdown: {specific_ae_count}")
