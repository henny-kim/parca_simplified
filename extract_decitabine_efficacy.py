#!/usr/bin/env python3
"""
Decitabine Clinical Efficacy Data Extraction for CMML Studies
Uses the same extraction logic as azacitidine but for decitabine papers
"""
...
def main():
    ...
    print("Starting clinical efficacy extraction for " + str(len(decitabine_papers)) + " decitabine papers...")
    print("Rate limiting: 5 requests/minute (12s intervals), 50 requests/day max")
    print("Estimated time: " + str(round(min(50, len(decitabine_papers)) * 12 / 60, 1)) + " minutes for first batch")
    print("Daily limit allows processing ~50 papers per day")

    # Check for existing checkpoint
    checkpoint_file = 'Decitabine_extraction_checkpoint.json'
    results = []
    start_index = 0
    ...
    for i, paper in enumerate(decitabine_papers[start_index:], start_index + 1):
        # No artificial limit - let it run until API quota is reached
            
        print("Processing paper " + str(i) + "/" + str(len(decitabine_papers)) + ": PMID " + str(paper.get('pmid')) + " (Request #" + str(requests_today + 1) + " today)")
        ...
        try:
            ...
            if "quota" in error_str.lower() or "limit" in error_str.lower() or "429" in error_str:
                ...
                print("Stopped at paper " + str(i) + "/" + str(len(decitabine_papers)))
        ...
        if i % 10 == 0:
            ...
            print("*** CHECKPOINT: " + str(i) + "/" + str(len(decitabine_papers)) + " papers processed ***")
            
        # Respect rate limits: 5 requests per minute = 12 second intervals
        if i < len(decitabine_papers):  # Don't wait after last paper
    ...
    print("\n" + "="*60)
    print("DECITABINE CLINICAL EFFICACY EXTRACTION COMPLETED")
    print("="*60)
