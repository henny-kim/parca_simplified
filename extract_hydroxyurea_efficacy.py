#!/usr/bin/env python3
"""
LLM-based Clinical Efficacy Data Extraction for Hydroxyurea CMML Studies
Extracts Complete Response (CR), Overall Response Rate (ORR), PFS, OS from studies
"""
...
class ClinicalEfficacyExtractor:
    ...
    def extract_clinical_efficacy(self, study: Dict[str, Any]) -> ClinicalEfficacyData:
        ...
        prompt = """
You are a medical data extraction expert. Analyze this research paper and extract clinical efficacy data related to CMML (Chronic Myelomonocytic Leukemia) treatment with HYDROXYUREA.
...
- Focus on HYDROXYUREA treatment for CMML patients
...
"""
        ...
        try:
            ...
            result = ClinicalEfficacyData(
                ...
                citation=citation,
                url=url,
                drug=study.get('drug', 'hydroxyurea'),
...
def main():
    ...
    # Load hydroxyurea papers
    with open('pubmed_hydroxyurea_cmml.json', 'r') as f:
        pubmed_data = json.load(f)
    
    hydroxyurea_papers = pubmed_data.get('hydroxyurea', [])
    print("Starting clinical efficacy extraction for " + str(len(hydroxyurea_papers)) + " hydroxyurea papers...")
    print("Rate limiting: 5 requests/minute (12s intervals), 50 requests/day max")
    print("Estimated time: " + str(round(min(50, len(hydroxyurea_papers)) * 12 / 60, 1)) + " minutes for first batch")
    print("Daily limit allows processing ~50 papers per day")

    # Check for existing checkpoint
    checkpoint_file = 'Hydroxyurea_extraction_checkpoint.json'
    results = []
    start_index = 0
    ...
    # Process papers with strict rate limiting - LIMIT TO 50 REQUESTS
    requests_today = start_index % 50  # Limit to 50 requests per day
    
    for i, paper in enumerate(hydroxyurea_papers[start_index:], start_index + 1):
        # Check daily limit - CHANGED FROM 100 TO 50
        if requests_today >= 50:
            print("*** DAILY LIMIT REACHED (50 requests) ***")
            print("Processed " + str(i-1) + "/" + str(len(hydroxyurea_papers)) + " papers")
            print("Switch to new API key to continue processing remaining " + str(len(hydroxyurea_papers) - (i-1)) + " papers")
            break
            
        print("Processing paper " + str(i) + "/" + str(len(hydroxyurea_papers)) + ": PMID " + str(paper.get('pmid')) + " (Request #" + str(requests_today + 1) + " today)")
        ...
        if i % 10 == 0:
            ...
            print("*** CHECKPOINT: " + str(i) + "/" + str(len(hydroxyurea_papers)) + " papers processed ***")
            
        # Respect rate limits: 5 requests per minute = 12 second intervals
        if i < len(hydroxyurea_papers) and requests_today < 50:  # Don't wait after last paper or when limit reached
    ...
    print("\n" + "="*60)
    print("HYDROXYUREA CLINICAL EFFICACY EXTRACTION COMPLETED")
    print("="*60)
