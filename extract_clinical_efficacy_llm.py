#!/usr/bin/env python3
"""
LLM-based Clinical Efficacy Data Extraction for CMML Studies
...
def main():
    ...
    for i, paper in enumerate(azacitidine_papers[start_index:], start_index + 1):
        ...
        if requests_today >= 50:
            ...
            print("Processed " + str(i-1) + "/" + str(len(azacitidine_papers)) + " papers")
            print("Switch to new API key to continue processing remaining " + str(len(azacitidine_papers) - (i-1)) + " papers")
            break
        ...
        if i < len(azacitidine_papers) and requests_today < 50:  # Don't wait after last paper or when limit reached
