#!/usr/bin/env python3
"""
Improved Clinical Outcomes Data Extractor for CMML Drugs
Enhanced pattern matching and data extraction
"""
...
class ImprovedClinicalExtractor:
    def __init__(self):
    ...
    def extract_clinical_data(self, abstracts: List[Dict], medicine: str) -> Dict:
        """Extract clinical data with improved pattern matching"""
        clinical_data = {
            'efficacy_measures': {
                'complete_response': [],
                'partial_response': [],
        ...
        for abstract in abstracts:
            ...
            # More flexible medicine detection
            ...
            # More flexible CMML detection
            cmml_found = ('cmml' in text or 'chronic myelomonocytic' in text or 
                         'chronic myelomonocytic leukemia' in text)
            
            if not medicine_found or not cmml_found:
                continue
            
            # Extract efficacy measures with improved patterns
            self._extract_efficacy_data_improved(text, abstract, clinical_data)
            
            # Extract adverse events
            self._extract_adverse_events_improved(text, abstract, clinical_data)
            
            # Extract RAS mutation data
            self._extract_ras_data_improved(text, abstract, clinical_data)
    ...
    def _extract_efficacy_data_improved(self, text: str, abstract: Dict, clinical_data: Dict):
        """Improved efficacy data extraction with more flexible patterns"""
        
        # Complete Response patterns - more flexible
        cr_patterns = [
            r'complete response.*?(\d+(?:\.\d+)?)\s*%',
            r'cr.*?(\d+(?:\.\d+)?)\s*%',
            r'complete remission.*?(\d+(?:\.\d+)?)\s*%',
            r'(\d+(?:\.\d+)?)\s*%.*?complete response',
            r'(\d+(?:\.\d+)?)\s*%.*?cr\b'
        ]
    ...
    def _extract_adverse_events_improved(self, text: str, abstract: Dict, clinical_data: Dict):
        """Improved adverse event extraction"""
        ae_patterns = [
            ...
            (r'(\d+(?:\.\d+)?)\s*%.*?neutropenia', 'Neutropenia'),
            (r'(\d+(?:\.\d+)?)\s*%.*?thrombocytopenia', 'Thrombocytopenia'),
            (r'(\d+(?:\.\d+)?)\s*%.*?anemia', 'Anemia')
        ]
    ...
    def _extract_ras_data_improved(self, text: str, abstract: Dict, clinical_data: Dict):
        """Improved RAS mutation data extraction"""
        ras_patterns = [
            ...
            (r'(\d+(?:\.\d+)?)\s*%.*?ras.*?mutated', 'ras_mutated'),
            (r'(\d+(?:\.\d+)?)\s*%.*?ras.*?mutation', 'ras_mutated')
        ]
...
def main():
    """Main function to run the improved clinical data extraction"""
    if len(sys.argv) < 2:
        print("Usage: python improved_clinical_extractor.py <medicine_name>")
        print("Available medicines: azacitidine, decitabine, hydroxyurea")
        sys.exit(1)
    
    medicine = sys.argv[1].lower()
    
    extractor = ImprovedClinicalExtractor()
    ...
    queries = [
        ...
        f"{medicine} CMML complete response",
        f"{medicine} CMML overall survival"
    ]
    ...
    with open(f"{medicine}_improved_report_{timestamp}.md", "w") as f:
        f.write(report)
    
    # Save raw data as JSON
    with open(f"{medicine}_improved_data_{timestamp}.json", "w") as f:
        json.dump(clinical_data, f, indent=2)
    
    print(f"Report saved as: {medicine}_improved_report_{timestamp}.md")
    print(f"Raw data saved as: {medicine}_improved_data_{timestamp}.json")
