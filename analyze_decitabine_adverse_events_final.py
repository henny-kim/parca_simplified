#!/usr/bin/env python3
"""
Analyze and count adverse events from extracted decitabine data
"""
...
def analyze_decitabine_adverse_events():
    # Load the extracted decitabine adverse events data
    with open('Decitabine_adverse_events_extracted.json', 'r') as f:
        data = json.load(f)
    ...
    for paper in data:
        ...
        if ae_text and ae_text != 'null':
            print(f"\nPMID {pmid}: {ae_text[:100]}...")
            all_adverse_events.append({
                'pmid': pmid,
                'text': ae_text,
                'patients': paper.get('number_of_patients', 1)
            })
    ...
    ae_patterns = {
        'myelosuppression': r'myelosuppression|cytopenia|cytopenias',
        'neutropenia': r'neutropenia|neutropenic',
        'thrombocytopenia': r'thrombocytopenia',
        'anemia': r'anemia|anaemia',
        'leukopenia': r'leukopenia',
        'gastrointestinal': r'gastrointestinal|nausea|vomiting|diarrhea|abdominal',
        'fatigue': r'fatigue|asthenia|tiredness|weakness',
        'infection': r'infection|sepsis|pneumonia|bacterial|viral|fungal',
        'febrile_neutropenia': r'febrile.*neutropenia|neutropenic.*fever',
        'skin': r'skin|rash|dermatitis|pruritus|sweet.*syndrome',
        'hepatotoxicity': r'hepatotoxicity|liver|hepatic|elevated.*liver|transaminase',
        'renal': r'renal|kidney|nephrotoxicity|creatinine',
        'cardiac': r'cardiac|heart|cardiomyopathy|heart.*failure',
        'bleeding': r'bleeding|hemorrhage|petechiae|ecchymosis',
        'transformation': r'transformation|aml|acute.*myeloid',
        'hearing': r'hearing|auditory|ototoxicity',
    ...
    print(f"\n📊 Decitabine Adverse Events Analysis:")
    ...
    for ae_type, data in adverse_event_counts.items():
        ...
        print(f"    Papers: {', '.join(data['papers'][:5])}{'...' if len(data['papers']) > 5 else ''}")
